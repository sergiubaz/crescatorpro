...
export default function NewRacePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="font-serif text-2xl font-bold text-gray-900">Adaugă concurs</h1>
        <p className="text-sm text-gray-500 mt-1">Completează datele concursului</p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <h2 className="font-semibold text-gray-900">Date generale</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Numele concursului *</label>
              <input {...register('name', { required: true })} className={inp} placeholder="ex: Campionatul Național Fond 2025" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Organizator</label>
              <input {...register('organizer')} className={inp} placeholder="ex: Federația Colombofilă Română" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Club</label>
              <input {...register('club')} className={inp} placeholder="ex: Club Colombofil Prahova" />
            </div>
            <div>
            ...
            <div>
              ...
              <input {...register('releaseLocation')} className={inp} placeholder="ex: Iași" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Distanță (km)</label>
              <input {...register('distanceKm')} type="number" step="0.1" className={inp} placeholder="ex: 480" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Categorie</label>
              <select {...register('category')} className={inp}>
                <option value="SPEED">Viteză</option>
                <option value="SEMI_DISTANCE">Demifond</option>
                <option value="DISTANCE">Fond</option>
        ...
        {/* Pigeon selection */}
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <h2 className="font-semibold text-gray-900">Porumbei angajați ({selectedPigeons.length})</h2>
          {pigeons.length === 0 ? (
