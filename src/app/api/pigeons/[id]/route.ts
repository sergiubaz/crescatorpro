...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.pigeon.findFirst({ where: { id, userId: session.user.id } })
  ...
  const duplicate = await prisma.pigeon.findFirst({
    where: { userId: session.user.id, ringNumber, id: { not: id } },
  })
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.pigeon.delete({ where: { id } })
