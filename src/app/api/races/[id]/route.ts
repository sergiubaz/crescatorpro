...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const race = await prisma.race.findFirst({
    ...
    include: {
      results: { include: { pigeon: true }, orderBy: { loftRank: 'asc' } },
    },
  })
  if (!race) return NextResponse.json({ error: 'Concurs negasit' }, { status: 404 })
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!existing) return NextResponse.json({ error: 'Concurs negasit' }, { status: 404 })
  ...
  if (results && Array.isArray(results)) {
    for (const result of results) {
      ...
      const existingResult = await prisma.raceResult.findFirst({ where: { raceId: id, pigeonId } })
      if (existingResult) {
        await prisma.raceResult.update({
          where: { id: existingResult.id },
          data: { ...resultData, arrivalTime: arrivalTime ? new Date(arrivalTime) : null, flightTimeMinutes, speedMetersPerMinute, classificationPercent },
        })
      } else {
        await prisma.raceResult.create({
          data: { raceId: id, pigeonId, ...resultData, arrivalTime: arrivalTime ? new Date(arrivalTime) : null, flightTimeMinutes, speedMetersPerMinute, classificationPercent },
        })
      }
    }
  }
